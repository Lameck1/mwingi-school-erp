...
export function up(db: Database) {
    console.error('Running migration: 014_correct_fee_structure')
    ...
    // Corrected Data from images (Amounts in CENTS)
    const FEE_DATA = [
        // === BABY, PP1, PP2 ===
        {
    ...
    try {
        const runReseed = db.transaction(() => {
            ...
            console.error(`Using Academic Year ID: ${acYearId}`)
                ...
                for (const code of group.classes) {
                    ...
                    for (const termData of group.terms) {
                        try {
                            ...
                            console.error(`Failed to insert fees for Class ${code}, Term ${termData.term}:`, itemErr.message)
                            throw itemErr
                        }
                    }
                }
            }
            console.error(`Inserted ${count} fee structure items.`)
        })

        runReseed()
    } catch (err: unknown) {
        console.error('Migration 014_correct_fee_structure FAILED:', err)
