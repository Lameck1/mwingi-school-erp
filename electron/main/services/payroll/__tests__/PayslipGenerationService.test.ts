...
import { PayslipGenerationService } from '../PayslipGenerationService'
...
describe('PayslipGenerationService', () => {
    let db: Database.Database
    let service: PayslipGenerationService

    beforeEach(() => {
        db = createTestDb()
        service = new PayslipGenerationService(db)
    })
    ...
    it('generates a highly structured payslip JSON for UI rendering', () => {
        const payslip = service.generatePayslip(1) // Alice

        expect(payslip.school_name).toBe('Test Academy')
        expect(payslip.period_name).toBe('January 2026')
        expect(payslip.payslip_id).toBe('PS-2026-01-EMP-001')

        expect(payslip.employee.name).toBe('Alice Teacher')
        expect(payslip.employee.kra_pin).toBe('A123456789Z')
        expect(payslip.employee.job_title).toBe('Senior Teacher')

        expect(payslip.earnings.basic_salary).toBe(50000)
        expect(payslip.earnings.gross_pay).toBe(60000)
        expect(payslip.earnings.allowances).toHaveLength(1)
        expect(payslip.earnings.allowances[0]?.name).toBe('Housing Allowance')
        expect(payslip.earnings.allowances[0]?.amount).toBe(10000)

        expect(payslip.deductions.total_deductions).toBe(9080)
        expect(payslip.deductions.items).toHaveLength(3)

        expect(payslip.net_pay).toBe(50920)
    })

    it('retrieves all payroll IDs for a batch period', () => {
        const ids = service.getPayrollIdsForPeriod(1)
