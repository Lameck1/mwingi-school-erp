...
import {
    ...
    ReportOptionalStreamSchema,
    ReportGenericFilterSchema,
...
function registerStudentAccountReports(db: ReturnType<typeof getDatabase>): void {
    validatedHandler('report:defaulters', ROLES.STAFF, z.number().optional(), (_event, termId) => {
...
function registerAttendanceAndCollectionReports(db: ReturnType<typeof getDatabase>): void {
    validatedHandlerMulti('report:attendance', ROLES.STAFF, ReportOptionalStreamSchema, (_event, [startDate, endDate, streamId]) => {
        let query = `
SELECT
s.id, s.admission_number, s.first_name, s.last_name,
    ...
    validatedHandler('report:dailyCollection', ROLES.STAFF, z.string().min(10), (_event, date) => {
        return db.prepare(`
SELECT
...
function registerFinancialSummaryAndDashboardReports(db: ReturnType<typeof getDatabase>): void {
    ...
    validatedHandlerMulti('report:financialSummary', ROLES.STAFF, ReportDateRangeSchema, (_event, [startDate, endDate]) => {
...
WHERE(transaction_type IN(${expenseTypesSql}))
    ...
    validatedHandler('report:dashboard', ROLES.STAFF, z.void(), () => {
...
function registerCategoryBreakdownReports(db: ReturnType<typeof getDatabase>): void {
    ...
    validatedHandlerMulti('report:revenueByCategory', ROLES.STAFF, ReportDateRangeSchema, (_event, [startDate, endDate]) => {
        return db.prepare(`
SELECT
COALESCE(tc.category_name, 'Uncategorized') as name,
    ...
    validatedHandlerMulti('report:expenseByCategory', ROLES.STAFF, ReportDateRangeSchema, (_event, [startDate, endDate]) => {
        return db.prepare(`
SELECT
COALESCE(tc.category_name, 'Uncategorized') as name,
    SUM(lt.amount) as value
    ...
    validatedHandler('report:feeCategoryBreakdown', ROLES.STAFF, z.void(), () => {
        return db.prepare(`
SELECT
...
function registerOperationsReports(db: ReturnType<typeof getDatabase>): void {
    validatedHandler('report:inventoryValuation', ROLES.STAFF, z.void(), () => {
        return db.prepare(`
SELECT
i.item_code, i.item_name, c.category_name,
    ...
    validatedHandler('report:staffPayroll', ROLES.STAFF, z.number(), (_event, periodId) => {
...
SUM(gross_salary) as total_gross,
    ...
    validatedHandlerMulti('report:feeCollection', ROLES.STAFF, ReportDateRangeSchema, (_event, [startDate, endDate]) => {
        return db.prepare(`
            SELECT
...
function registerNemisExportHandlers(): void {
    ...
    validatedHandlerMulti('reports:createNEMISExport', ROLES.MANAGEMENT, z.tuple([NEMISExportConfigSchema, z.number().optional()]), async (_event, [exportConfig, legacyId], actor) => {
    ...
    validatedHandler('reports:getNEMISExportHistory', ROLES.STAFF, z.number().optional(), async (_event, limit) => {
        try {
    ...
    validatedHandler('reports:validateNEMISStudentData', ROLES.STAFF, NEMISStudentSchema, (_event, student) => {
        try {
