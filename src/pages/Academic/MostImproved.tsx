...
type MostImprovedContentProps = Readonly<{
  loading: boolean;
  improvedStudents: ImprovedStudent[];
  minimumImprovement: number;
}>;

function MostImprovedContent({ loading, improvedStudents, minimumImprovement }: MostImprovedContentProps) {
  if (loading) {
    return (
...
type AwardActionsPanelProps = Readonly<{
  selectedAward: string;
  onAwardChange: (val: string | number) => void;
  awardCategories: { value: string; label: string }[];
  onAwardCertificates: () => void;
  onEmailParents: () => void;
  onExportList: () => void;
}>;

function AwardActionsPanel({
  selectedAward,
  onAwardChange,
  awardCategories,
  onAwardCertificates,
...
const MostImproved = () => {
    ...
    try {
      if (currentAcademicYear) {
        const [termsData, streamsData] = await Promise.all([
          globalThis.electronAPI.academic.getTermsByYear(currentAcademicYear.id),
  ...
  const handleGenerateMostImproved = async () => {
    ...
    try {
      const students = unwrapArrayResult(
        await globalThis.electronAPI.academic.getMostImprovedStudents({
    ...
    try {
      ...
      await Promise.all(
        improvedStudents.map(async (student) => unwrapIPCResult(
          await globalThis.electronAPI.academic.generateCertificate({
  ...
  const handleEmailParents = async () => {
    ...
    try {
      ...
      unwrapIPCResult(
        await globalThis.electronAPI.academic.emailParents({
    ...
    <div className="space-y-8 pb-10">
      ...
      {improvedStudents.length > 0 && (
        <AwardActionsPanel
          ...
          onAwardChange={(val) => setSelectedAward(String(val))}
          awardCategories={awardCategories}
          onAwardCertificates={handleAwardCertificates}
          onEmailParents={handleEmailParents}
